...
async function scrapeDataForInidia() {
  try {
    ...
    const casesSection = $('#state-data');
    const table = $(casesSection).find("tbody");
    let rows = [];
    $("tr", table).map((i, el) => {
      const cols = [];
      $("td", el).map((i, el) => {
        ...
        if (stateMeta) {
          statesCases.push({
            ...
            data: {
              ...
              existing: parseFloat(stateStats["confirmed"]) - (parseFloat(stateStats["deaths"])+parseFloat(stateStats["recovered"])),
