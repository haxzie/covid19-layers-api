...
async function generateLayerData() {
    ...
    if (status === 200 && data) {
        ...
        const indexedConfirms = await createIndexes({ data: fixCountryCodes(confirmed.locations), keys: ["country_code", "province"] });
        
        sp_layers.text = "Indexing death cases";
        const indexedDeaths = await createIndexes({ data: fixCountryCodes(deaths.locations), keys: ["country_code", "province"] });
        
        sp_layers.text = "Indexing recovered cases";
        const indexedRecovered = await createIndexes({ data: fixCountryCodes(recovered.locations), keys: ["country_code", "province"] });
        
        sp_layers.succeed();

        let sp_india = ora("Fethcing country data for India...").start();
        const indexedIndiaData = await createIndexes({ data: await generateIndiaData(), keys: ["country_code", "province"]});
        sp_india.succeed();
        ...
        layerData.push(...Object.keys(indexedIndiaData).map(id => indexedIndiaData[id]));
        sp_merging.succeed();
